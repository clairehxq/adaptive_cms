...
private:
  ...
  int extended =0;
  int extend_limit;
  std::unordered_set<uint32_t> extended_set;
  void init(uint32_t W, uint32_t D, uint32_t R, uint32_t P, uint32_t T, int L){
...
public:
  int global_c = 0;
  uint32_t max_v=0;
  adaptive_cms(uint32_t W, uint32_t D, uint32_t R, uint32_t P, uint32_t T, int L){
    this->init(W, D, R, P, T, L);
  }
  ...
  void addDeep(uint64_t value) {
    ...
    for (uint32_t i=0; i<D; ++i){
      uint32_t j = this->hash(value, i);
      ++ this->c[(i*W) + j];
      if ((this->c[i*W + j] > T) && (this->extended < this->extend_limit)){
	this->extended_set.insert((j<<3+i));
  ...
  uint32_t countMinDeep(uint64_t value) const {
    ...
    for (uint32_t i=0; i<D; ++i){
      ...
      if ((this->c[i*W + j] <= T) || (this->extended_set.find((j<<3)+i) == this->extended_set.end())){
